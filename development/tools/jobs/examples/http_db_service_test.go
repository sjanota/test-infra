...
func TestHttpDbServiceJobsPresubmit(t *testing.T) {
	...
	expName := "pre-master-examples-http-db-service"
...
func TestHttpDbServiceJobPostsubmit(t *testing.T) {
	...
	expName := "post-master-examples-http-db-service"
	...
	assert.Equal(t, []string{"/home/prow/go/src/github.com/kyma-project/examples/http-db-service"}, actualPost.Spec.Containers[0].Args)
}
